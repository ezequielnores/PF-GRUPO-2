...
const postDoctor = async (
  ...
  try {
    if (image)
      var uploadedImageResponse = await cloudinary.uploader.upload(image, {
        upload_preset: "iCare_Henry",
      });
    const uploadedCvResponse = await cloudinary.uploader.upload(cv, {
      upload_preset: "iCare_Henry",
    });
    ...
    const response = await Doctor.create({
      ...
      password,
      birthdate,
      image: uploadedImageResponse ? uploadedImageResponse.url : null,
      location,
      dni,
      phone,
      speciality,
...
const setDoctorActive = async (id) => {
  ...
  await Doctor.update(
    { active: !doctorToSetActive.active },
    { where: { id: id } }
  );
